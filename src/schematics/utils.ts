...
export function getFirebaseProjectName(projectRoot: string, target: string): string|undefined {
    const {targets}: FirebaseRc = JSON.parse(readFileSync(join(projectRoot, '.firebaserc'), 'UTF-8'));
