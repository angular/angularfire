...
import { RemoteConfig, Value as RemoteConfigValue, ValueSource, Settings } from '@firebase/remote-config-types';
...
export class Value implements RemoteConfigValue {
  asBoolean() {
...
): OperatorFunction<{ [key: string]: RemoteConfigValue }, Parameter[]> => pipe(
...
export class AngularFireRemoteConfig {
  ...
  constructor(
    ...
    @Optional() @Inject(FIREBASE_APP_NAME) name: string | null,
    ...
    const remoteConfig$ = of(undefined).pipe(
      ...
      switchMap(() => isPlatformBrowser(platformId) ?
        import(/* webpackExports: ["registerRemoteCofing"] */ 'firebase/remote-config') :
        EMPTY
      ),
      map(({getRemoteConfig }) => {
        const app = ɵfirebaseAppFactory(options, zone, platformId, name, undefined);
        return ɵfetchInstance(`${app.name}.remote-config`, 'AngularFireRemoteConfig', app, () => {
    ...
    const filterOutDefaults = map<{ [key: string]: RemoteConfigValue }, { [key: string]: RemoteConfigValue }>(all =>
      Object.keys(all)
        .filter(key => all[key].getSource() !== 'default')
        .reduce((acc, key) => ({ ...acc, [key]: all[key] }), {})
    );

    this.parameters = of(undefined).pipe(
      switchMap(() =>
        import(/* webpackExports: ["fetchAndActivate", "ensureInitialized", "getAll", "activate"] */ 'firebase/remote-config')
      ),
      switchMap(({ fetchAndActivate, ensureInitialized, getAll, activate }) => {

        const default$: Observable<{ [key: string]: RemoteConfigValue }> = of(Object.keys(defaultConfig || {}).reduce(
        ...
        return concat(default$, existing$, fresh$);
      }),
